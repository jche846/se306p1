...
namespace se306p1 {
  Supervisor::Supervisor() {
    ...
    ansPosSubscriber_ = nh_.subscribe<Position>(
        ANS_POS_TOPIC, 1000, &Supervisor::ansPos_callback, this,
    ...
    while (poses.size()) {
      ...
      for (size_t nodeIndex = 0; nodeIndex < nodes.size(); nodeIndex++) {
        ...
        for (size_t posesIndex = 0; posesIndex < poses.size(); posesIndex++) {
          double testDist =
              abs((node->pose_.position_ - poses[posesIndex].position_).LengthSquared());
          if (testDist < dist) {
            dist = testDist;
